...
export function IngredientsClient() {
  ...
  const { ingredients, menus, addIngredient, updateIngredient, removeIngredient, loadSampleData } = useData();
  ...
  const usageCount = useMemo(() => {
    ...
    for (const menu of menus) {
      const ids = new Set(menu.items.map((item) => item.ingredientId).filter(Boolean) as string[]);
